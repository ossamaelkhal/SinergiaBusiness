...
const FlowDemo: React.FC<FlowDemoProps> = ({ isOpen = true, onClose, isInline = false, nicheSlug, onInteraction, onCompilar }) => {
    ...
    const getNicheTemplate = (slug?: string) => {
        if (!slug) return null;
        switch (slug) {
            case 'faturamento-saude-bemestar':
            case 'saude':
                return flowTemplates.find(f => f.id === 'staff-scheduling') || null;
            case 'commerce-omnichannel-vendas':
            case 'varejo':
                return flowTemplates.find(f => f.id === 'order-quoting') || null;
            case 'operacoes-urgencia-logistica':
            case 'logistica':
                return flowTemplates.find(f => f.id === 'customer-support') || null;
            case 'bpo-financeiro-credito-tem':
            case 'bpo':
            case 'financas':
                return flowTemplates.find(f => f.id === 'active-billing') || null;
    ...
    useEffect(() => {
        if (nicheSlug) {
            const template = getNicheTemplate(nicheSlug);
            if (template) {
    ...
    }, [isSimulating, currentStep, selectedFlow, calculateResults, onInteraction])
    ...
    const innerContent = (
        <div className={`relative bg-slate-900/40 backdrop-blur-2xl border border-white/10 rounded-3xl w-full overflow-y-auto shadow-[0_0_50px_rgba(99,102,241,0.15)] flex flex-col ${isInline ? 'h-[75vh]' : 'max-w-6xl max-h-[90vh]'}`}>
            {/* Header */}
