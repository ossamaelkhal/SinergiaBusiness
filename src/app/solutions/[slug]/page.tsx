...
import { ArrowLeft, CheckCircle2, ChevronRight, MessageSquare, PlayCircle, ShieldCheck, Zap } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50 selection:bg-slate-800">
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div className="max-w-2xl">
              <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full ${bgMaps[niche.color]} ${borderMaps[niche.color]} border text-sm font-bold ${textMaps[niche.color]} mb-8 uppercase tracking-wider`}>
                ...
                <span>Playbook Especializado</span>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <Button className={`h-14 px-8 rounded-xl font-black text-lg ${buttonMaps[niche.color]}`}>
                  Quero Implantar Essa IA
            ...
            <div className="relative aspect-square md:aspect-[4/3] rounded-3xl overflow-hidden border border-white/10 bg-slate-900/50 backdrop-blur-xl shadow-2xl flex flex-col items-center justify-center group p-8">
               ...
               <div className="relative z-10 bg-slate-950/80 backdrop-blur-md border border-white/10 p-5 rounded-2xl w-full translate-y-4 group-hover:translate-y-0 transition-transform duration-500 flex items-center justify-between">
                  <div>
                    <div className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Custo de Licença</div>
                    <div className="text-2xl font-black text-white">R$ 297<span className="text-sm font-medium text-slate-500">/mês</span></div>
                  </div>
                  <Link href="/loja/checkout">
                     <Button className={`h-12 rounded-xl font-bold ${buttonMaps[niche.color]}`}>Ativar Setup</Button>
                  </Link>
               </div>
            </div>
          </div>
        ...
        <section className="bg-slate-900/40 border-y border-white/5 py-24 relative overflow-hidden">
                 ...
                 {niche.solutions.map((sol, idx) => (
                   <div key={idx} className={`bg-gradient-to-br from-white/5 to-white/0 border rounded-2xl p-6 ${borderMaps[niche.color]} relative overflow-hidden group`}>
                     ...
                     <div className="relative z-10">
                       ...
                       <p className="text-slate-300 leading-relaxed mb-4">{sol.description}</p>
                       
                       <div className="mb-6 py-3 border-y border-white/10">
                         <div className={`text-sm font-bold ${textMaps[niche.color]}`}>Impacto Direto:</div>
                         <div className="text-white font-medium">{sol.impact}</div>
                       </div>
                       
                       <ul className="space-y-2">
                         {sol.features.map((feat, fidx) => (
                           <li key={fidx} className="flex items-center text-sm text-slate-400">
                             <CheckCircle2 className={`w-4 h-4 mr-2 ${textMaps[niche.color]}`} />
                             {feat}
                           </li>
                         ))}
                       </ul>
        ...
        {/* CTA Section - The Dual Funnel */}
        <section className="container mx-auto px-4 py-32">
          <div className="text-center max-w-3xl mx-auto mb-16">
             <h2 className="text-4xl md:text-5xl font-black mb-6">Pronto para dominar seu nicho?</h2>
             <p className="text-xl text-slate-300">
                Escolha como você quer começar. Implemente você mesmo com nosso playbook ou deixe nossos engenheiros instalarem a máquina completa.
             </p>
          </div>

          <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto align-stretch">
            {/* Self-Service Plan */}
            <div className="bg-slate-900/60 border border-white/10 rounded-[2.5rem] p-10 flex flex-col hover:bg-white/5 transition-colors">
               <div className="mb-6">
                 <div className="inline-flex items-center px-3 py-1 bg-white/5 border border-white/10 rounded-full text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Self-Service</div>
                 <h3 className="text-3xl font-black text-white mb-2">Playbook {niche.shortTitle}</h3>
                 <p className="text-slate-400">Template completo para N8N, Typebot e Prompts do seu nicho prontas para plugar.</p>
               </div>
               
               <div className="mb-8">
                 <span className="text-5xl font-black text-white">R$ 297</span>
                 <span className="text-slate-400"> / vitalício</span>
               </div>

               <div className="space-y-4 mb-10 flex-1">
                 <div className="flex items-center text-sm text-slate-300"><CheckCircle2 className="w-5 h-5 mr-3 text-slate-500" /> Blueprints N8N (JSON)</div>
                 <div className="flex items-center text-sm text-slate-300"><CheckCircle2 className="w-5 h-5 mr-3 text-slate-500" /> Fluxos Typebot Prontos</div>
                 <div className="flex items-center text-sm text-slate-300"><CheckCircle2 className="w-5 h-5 mr-3 text-slate-500" /> Engenharia de Prompt Especializada</div>
                 <div className="flex items-center text-sm text-slate-300"><CheckCircle2 className="w-5 h-5 mr-3 text-slate-500" /> Acesso ao Dashboard SinergIA</div>
               </div>

               <Link href="/loja/checkout" className="w-full mt-auto">
                 <Button className="w-full h-16 rounded-2xl font-bold bg-white/10 hover:bg-white/20 text-white border border-white/20">
                    Comprar Playbook Agora
                 </Button>
               </Link>
            </div>
            ...
            <div className={`bg-gradient-to-br from-white/5 to-white/0 border ${borderMaps[niche.color]} rounded-[2.5rem] p-10 flex flex-col relative overflow-hidden`}>
               ...
               <div className="relative z-10">
                 <div className="inline-flex items-center px-3 py-1 bg-white/10 backdrop-blur-md border border-white/20 rounded-full text-xs font-bold text-white uppercase tracking-widest mb-4">
                    Recomendado SinergIA
                 </div>
                 <h3 className="text-3xl font-black text-white mb-2">Enterprise + Setup</h3>
                 <p className="text-slate-300 mb-6">Implantação completa &quot;Done-for-You&quot;. Nossa equipe arquitetura a IA nos servidores do seu CRM e site.</p>
                 
                 <div className="mb-8 p-4 bg-slate-950/50 rounded-xl border border-white/10">
                    <div className="text-sm font-bold text-white mb-1">O que está incluso:</div>
                    <p className="text-xs text-slate-400">Tudo do pacote Self-Service + Integração de APIs nativas, Reuniões semanais e Garantia Estendida de Setup SinergIA.</p>
                 </div>
               </div>
